...
Create `src/plugins/metadata.ts`:
...
import { metadataPlugin } from "./src/plugins/metadata.js";
...
Receives one scanned `FolioPage` before route and navigation structures are
...
Before Folio 1.0, use a compatible minor range such as `~0.14.0`. After 1.0,
